...
namespace Drupal\sitestudio_contenthub_publisher\EventSubscriber\DependencyCollector;
 ...
 */
class SitestudioLayoutCanvasDependencyCollector extends BaseDependencyCollector {
  ...
  /**
   * SitestudioJsonValueFieldDependencyCollector constructor.
   *
  ...
  protected function loadEntity(array $entityReference) {
    $entity_type = $entityReference['entity_type'] ?? NULL;
    if (!is_string($entity_type) || $entity_type === '' || !$this->entityTypeManager->hasDefinition($entity_type)) {
      return FALSE;
    }
    ...
    if (Uuid::isValid($entityReference['entity_id'])) {
      $results = $this->entityTypeManager
        ...
        ->loadByProperties(['uuid' => $entityReference['entity_id']]);
      $entity = reset($results);
    }
    else {
      $entity = $this->entityTypeManager->getStorage($entity_type)->load($entityReference['entity_id']);
    }
