 ...
 */
abstract class SitestudioContentHubImportExportTestCase extends ImportExportTestBase {
  ...
  protected function getFixtureString(int $delta) {
    if (!empty($this->fixtures[$delta])) {
      $path_to_fixture = sprintf("%s/tests/fixtures/import/{$this->getFixtureDirectory()}/%s",
        $this->container->get('extension.path.resolver')->getPath('module', 'sitestudio_contenthub'),
        $this->fixtures[$delta]['cdf']
      );
      $data = file_get_contents($path_to_fixture);
  ...
  /**
   * Return the fixture directory in the format major.minor
   *
   * @return string
   */
  protected function getFixtureDirectory() {
    $version = explode('.', $this->getDrupalVersion(TRUE));
